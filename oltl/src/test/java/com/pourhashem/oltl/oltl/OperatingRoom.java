...
//		result.addAll(getSurgeonsSurgeries(2, 1, 2, 7));
	 ...
	 */
	private ArrayList<LTLFormula> getSurgeonsSurgeries(int ... snSrs) {
		ArrayList<LTLFormula> result = new ArrayList<LTLFormula>();
		assert (snSrs.length > 1);
		for (int orIndex = 1; orIndex <= numberOfORs; orIndex++) {
			Or surgeriesOr = new Or();
			for (int srIndex = 1; srIndex < snSrs.length; srIndex++) {
				surgeriesOr.add(getORSRPredicate(orIndex, snSrs[srIndex]));
			}
			result.add(new Impl(getORSNPredicate(orIndex, snSrs[0]), surgeriesOr));
		}
  ...
  * (&& (i allOR) (&& (j allSN)
  ...
  //no unwanted SNs
	...
	*/
	...
	/*SR7P	SN2	SR7C
	*/
	private ArrayList<LTLFormula> getSR7Axioms() {
		ArrayList<LTLFormula> result = new ArrayList<LTLFormula>();
		int srIndex = 7;
		for (int orIndex = 1; orIndex <= numberOfORs; orIndex++) {
			And and = new And();
	...
	/*SR8P	SN2	SR8C
	*/
	private ArrayList<LTLFormula> getSR8Axioms() {
		ArrayList<LTLFormula> result = new ArrayList<LTLFormula>();
		int srIndex = 8;
		for (int orIndex = 1; orIndex <= numberOfORs; orIndex++) {
			And and = new And();
	...
	/*SR9P	SN2	SR9C
	*/
	private ArrayList<LTLFormula> getSR9Axioms() {
		ArrayList<LTLFormula> result = new ArrayList<LTLFormula>();
		int srIndex = 9;
		for (int orIndex = 1; orIndex <= numberOfORs; orIndex++) {
			And and = new And();
...
*/
	...
	private ArrayList<LTLFormula> getSoftNoOverTimeSN() { //can be optimized, using softAt @13 @14 @15, instead of softAll.
		...
		for (int snIndex = 1; snIndex <= numberOfSurgeons; snIndex++) {
			...
			softFormulae.add(new SoftAll(softNoOverTimeSNiP, noOverTimeSNPenalty[snIndex - 1]));
		}
	...
	private ArrayList<LTLFormula> getSoftNoWaitingSN() {
		...
		for (int snIndex = 1; snIndex <= numberOfSurgeons; snIndex++) {
			...
			softFormulae.add(new SoftAll(softNoWaitingSNiP, noWaitingSNPenalty[snIndex - 1]));
		}
