...
class SMT2Builder {
	...
	public SMT2Builder(String script, int bound, int m) {
		this.script = script;
		this.m = m;
		k = bound;
	...
	private String getFixedPart() {
		String fixedPart = "";
		model = new Model(script, k, m);
		this.k = Model.getBound();
		boolean realAvgCost = true;
		if (realAvgCost)
			fixedPart += "(declare-const costHSum Real)\n"
					+ "(declare-const costHAvg Real)\n"
					+ "(declare-const costLSum Real)\n";

		fixedPart += "(declare-const i_loop (_ BitVec " + (k+2) + "))\n"
				+ "(declare-const in_loop (_ BitVec " + (k+2) + "))\n"
	...
	private String getFixedPartEnd() {
		String s = "";
		if (model.getHasSoftConstraints())
			s = "\n(get-objectives)";
